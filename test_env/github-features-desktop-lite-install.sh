...
NOVNC_VERSION="${VERSION:-"${NOVNCVERSION:-"1.6.0"}"}"
...
# Check if we need to auto-detect the version
if [ "${NOVNC_VERSION}" = "latest" ] && [ "${INSTALL_NOVNC}" = "true" ]; then
    # We need curl and ca-certificates which are guaranteed to be installed by check_packages above
    echo "Auto-detecting latest noVNC version..."
    LATEST_VERSION=$(curl -sSL "https://api.github.com/repos/novnc/noVNC/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
